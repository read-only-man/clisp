(setf *terminal-encoding* charset:shift-jis)
...
(defun decode-param (s)
  (labels ((f (lst)
    ...
    (ext:convert-string-from-bytes
      (coerce (f (coerce (ext:convert-string-to-bytes s charset:shift-jis) 'list))
	      'vector)
      charset:shift-jis)))
...
(defun hello-request-handler (path header params)
  (if (equal path "greeting")
    (let ((name (assoc 'name params)))
      (if (not name)
	(princ "<html><form>What is your name?<input type='name' /></form></html>")
